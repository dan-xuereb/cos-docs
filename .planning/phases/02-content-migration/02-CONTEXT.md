...
### Reconciliation With Disk State (added 2026-04-19, post-research)
- **D-14:** **Authoritative repo list = 31 repos on disk in `/home/btc/github/`, MINUS `COS-electrs` (Rust, out of stack) = 30 repos.** D-11 ("CLAUDE.md verbatim") is amended: research found 8 disk repos absent from the project map (COS-MSE, cos-data-access, cos-signal-explorer, cos-webpage, COS-electrs, COS-Infra, COS-BTC-Node, stooq-forge) and 3 renamed (`fred-forge`→`FRED-Forge`, `edgar-forge`→`EDGAR-Forge`, `bitcoin_node`→`COS-BTC-Node`). All 7 non-Rust omissions are included; the planner MUST add a task to update `/home/btc/github/CLAUDE.md`'s project map to match disk reality (and reflect renames).
- **D-15:** Wrapper branch policy: scaffold on **`main` OR `master`** (treat as equivalent — `master` is the historical alias). Repos on any other branch (`kubernetes`, `spec-decomposition-extraction`, `new-index`) are **skipped with a logged reason**; the user manually checks out main/master and re-runs `scaffold.sh /path/to/repo` against that single repo. Wrapper never auto-switches branches.
- **D-16:** Wrapper dirty-tree policy: if `git status --porcelain` is non-empty in a target repo, **skip with warning, never auto-stash.** The user decides whether to commit/stash/clean before re-running. Zero risk of clobbering in-progress work in the 14 dirty sibling repos.
- **D-17:** Package-name overrides via **hardcoded `PACKAGE_OVERRIDES` map in `scaffold-all.sh`** — distribution name in `pyproject.toml` does not always match the importable module name (verified for `COS-LangGraph`: distribution `cos-langgraph` → module `langgraph_agent`). Planner spot-checks all 19 Python repos at plan time and seeds the map. Wrapper passes the override to `scaffold.sh` via a new `--package <name>` flag (small Phase 1 amendment within scope of plan 02-01) or by post-editing the generated `api.md`. Without an override, `mkdocs build --strict` fails on import-resolution.
