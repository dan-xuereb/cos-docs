...
flowchart LR
    ...
    CHW --> QD
    LG --> QD
    CORE -.canonical models.-> FRG
    CORE -.canonical models.-> SIG
    NET -.topology.-> QD
    INF -.deploys.-> QD
    INF -.deploys.-> LG
```
...
- 8 subgraphs = 8 domain nodes per D-13.
- ~10 critical arrows total (D-13 targeted 5-6 — the 10 here are all "critical workspace flows"; if user feedback wants it narrower, they can trim during review).
- Links point to aggregator-relative paths (e.g., `BTC-Forge/architecture/`), NOT absolute (Pitfall 7).
  </action>
  <verify>
    <automated>test -f /home/btc/github/cos-docs/docs/architecture.md && grep -q '^# Workspace Architecture' /home/btc/github/cos-docs/docs/architecture.md && grep -q '^```mermaid' /home/btc/github/cos-docs/docs/architecture.md && grep -c 'subgraph ' /home/btc/github/cos-docs/docs/architecture.md | awk '{if ($1 < 8) exit 1}' && ! grep -qE '\]\(/' /home/btc/github/cos-docs/docs/architecture.md</automated>
  </verify>
  <acceptance_criteria>
    ...
    - Contains at least 8 `subgraph ` labels (one per domain)
    - Contains at least 5 arrow operators (`-->` or `-.->`)
