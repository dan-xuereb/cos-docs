...
- **Phase:** 4 — Deploy & CI (4/5 plans complete)
- **Plan:** 04-04 (`cab97f6` + `4934189` on main) complete; `.github/workflows/build.yml` shipped and verified green via workflow_dispatch run 24673734857 (1m44s). Registry `10.70.0.30:5000/cos-docs` now holds tags `:4934189` + `:latest`. CI-01/CI-02/CI-03 all satisfied.
- **Status:** 04-05 (E2E deploy verification — `kubectl apply -k` + curl assertions at 10.70.0.102:30083) is the final Phase 4 plan. Phase 2 plan 02-03 (per-repo content authoring) remains independently pending.
- **Progress:** [■■■■■] Phase 4 at 4/5; CI pipeline live; only DEPLOY-04 live-cluster verification remains.
...
| 04-04 | ~2100 | 2 | 1 created (.github/workflows/build.yml) + 1 in-flight fix to same file | 2 main (cab97f6, 4934189) + GH run 24673734857 (1m44s green; registry tags :4934189 + :latest) |
...
- CI-01/02/03 all closed: `.github/workflows/build.yml` shipped with `schedule: [{cron: '0 7 * * *'}]` + `push: { branches: [main] }` + `workflow_dispatch` with `allow_partial` boolean; workflow_dispatch run 24673734857 green in 1m44s; registry `10.70.0.30:5000/cos-docs` holds `:4934189` + `:latest`.
- site-manifest.json location is repo-root, NOT `site/`. The Dockerfile `COPY site-manifest.json /usr/share/nginx/html/site-manifest.json` reads from build-context root; an initial `mv site-manifest.json site/` in the workflow caused `COPY failed: not found in build context` on first dispatch (run 24673509302 red). Fix in commit `4934189`.
- `:nightly-YYYYMMDD` tag intentionally emits only when `github.event_name == 'schedule'` — operator-triggered dispatches produce only `:<short-sha>` + `:latest`, which is correct per D-03 (nightly tag denotes scheduled provenance, not any recent build).
- In-place workspace sync pattern locked in: `git fetch origin && git reset --hard "$GITHUB_SHA" && git clean -fdx -e site/ -e .venv-docs/ -e .venv-aggr/`. NO `actions/checkout` (would clone into `_work/` and break sibling-repo access). `defaults.run.working-directory: /home/btc/github/cos-docs` ties every step to the real workspace.
- Concurrency group `cos-docs-build` with `cancel-in-progress: false` serializes nightly + push + dispatch so the sibling docs/api.md swap/restore dance in `build-all-api.sh --keep` / `--restore` never races (T-04-04-02).
...
- **CI-01 / CI-02 / CI-03**: RESOLVED by 04-04 (`cab97f6` + `4934189` on main) — `.github/workflows/build.yml` shipped; workflow_dispatch run 24673734857 green in 1m44s; registry tags `:4934189` + `:latest` confirmed via `curl http://10.70.0.30:5000/v2/cos-docs/tags/list`.
...
**Next action:** Phase 4 plan 04-04 complete (`.github/workflows/build.yml` on main @ cab97f6 + 4934189; workflow_dispatch run 24673734857 green in 1m44s; registry `10.70.0.30:5000/cos-docs` holds `:4934189` + `:latest`; CI-01/CI-02/CI-03 closed). Next up: 04-05 (E2E deploy verification — `kubectl apply -k cos-docs/k8s/` + curl assertions at http://10.70.0.102:30083/ + DEPLOY-VERIFICATION.md transcript). No blockers.
**Files in play:**
